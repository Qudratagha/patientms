        ...
        <div class="card-body">
			<form class="form-horizontal" action="{{ route('role.update',$role->roleID) }}" method="POST">
                ...
                <div class="form-group mb-2 row {{ $errors->has('roleName') ? 'has-error' : '' }}">
                    <label for="roleName" class="col-sm-2 required form-label">{{__('menu.role_name')}}:</label>
					<div class="col-sm-10">
				...
				<div class="form-group row mb-2">
                    <label for="description" class="col-sm-2 form-label">{{__('menu.description')}}: </label>
					<div class="col-sm-10">
						...
						<table class="table" @if($errors->has('privilegeID')) style="border:1px solid red;" @endif>
							<thead>
								<tr>
									...
									<th colspan="4">{{__('menu.privileges')}}</th>
								</tr>
							</thead>
							<tbody>
								@php
